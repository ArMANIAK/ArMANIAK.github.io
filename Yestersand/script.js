...
const gameOver = (reason) => {
    ...
    if (reason == 1) {
        console.log('reason - coronavirus');
        element.innerText = 'Эпидемия побеждена ценой ' + STATISTICS.deaths + ' жизней';
    }
    else {
        console.log('reason - panic');
        element.innerText = 'Игра окончена! Вы умерли из-за ' + reason; 
        //Вы стали ' + STATISTICS.deaths + ' жертвой эпидемии';
    }
    // printStatistics();
    console.log(reason);
    console.log(element);
...
// ПАНИЧЕСКАЯ АТАКА
        ...
        if (object.view == 'infectious' && neighbour.view == 'panic') {
            if (isSuccess(1 - (neighbour.immunity - object.immunity))) {
                ...
                STATISTICS.infectious--;
                STATISTICS.deaths++;
                map[object.y_coord][object.x_coord].filled = null;
                debugChat.innerHTML += ('<p>Еще один человек пал жертвой паники!</p>');
...
// ЗАРАЖЕНИЕ
        ...
        if ((neighbour.view == 'clean' || neighbour.view == 'panic') && object.view == 'infectious') {
...
// ПРОВЕРКА СОСЕДЕЙ
    ...
    map[person.y_coord][person.x_coord].filled = JSON.stringify(person);
            ...
            if (tile.filled) {
                ...
                if (hero.x_coord != column && hero.y_coord != row) {
                    if (alreadyMoved.indexOf(person == -1)) {
                        ...
                        alreadyMoved.push(JSON.stringify(person));
                    }
                }
            }
        }
...
let hero = {x_coord: 3, y_coord: 2};
new Character(3, 2, 'clean');
...
document.body.addEventListener('keypress', event => {
    let player = JSON.parse(map[hero.y_coord][hero.x_coord].filled);
